...
module VCAP::Services::Base::ProvisionerV3
      ...
      if version_nodes.count > 0
        ...
        if best_nodes.size > 0 && ( allow_over_provisioning ||
          ...
          if is_restoring
            ...
            prov_properties = gen_provision_properties({:is_restoring => false})
            provision_on_node(svc, plan, version, prov_properties, blk)
          end
  ...
  def trigger_restore(svc, plan, version, recipes, backup_id, original_service_id, blk)
    ...
    peers = svc[:configuration]["peers"]
    prov_properties = gen_provision_properties({:is_restoring => true})
      ...
      begin
        if sr.success
          ...
          if count == 0
            ...
            provision_on_node(svc, plan, version, prov_properties, update_status_blk)
            @node_nats.unsubscribe(subscription) if subscription
            EM.cancel_timer(timer)
          end
        else
  ...
  def provision_on_node(svc, plan, version, prov_properties, callback)
    service_id = svc[:service_id]
    peers = svc[:configuration]["peers"]
    peers.each do |peer|
      creds = peer["credentials"]
  ...
  def gen_provision_properties(properties)
    properties
  end
