...
module VCAP::Services::Base::ProvisionerV3
  ...
  def provision_service(request, &blk)
    ...
    if request.respond_to?(:properties) && request.properties.kind_of?(Hash)
      ...
      raise 'Update URL is not given in the restore request' if (is_restoring && (update_url.length rescue 0) == 0)
    end
  ...
  before [:create_backup, :delete_backup], :before_backup_apis
end
